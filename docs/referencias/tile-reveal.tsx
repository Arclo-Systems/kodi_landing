...
const TileReveal = ({
  ...
  const plan = useMemo(() => {
    ...
    const tiles = Array.from({ length: count }, (_, i) => {
      ...
      const len = lengths[c];
      const fromTop = direction === 'top' ? true : direction === 'bottom' ? false : c % 2 === 0;
  ...
  const paint = useCallback(
    (progress: number) => {
      ...
      const tileW = Math.max(1, (gridWidthNow.current - gap * (cols - 1)) / cols);
      ...
      const clearX = (colOffset: number) =>
        Math.max(
          spread * tileW,
          stageWidth.current / (2 * scale) + tileW / 2 - Math.abs(colOffset) + gap,
        );
      const clearY = (rowOffset: number) =>
        Math.max(
          spread * tileH,
          stageHeight.current / (2 * scale) + tileH / 2 - Math.abs(rowOffset) + gap,
        );
  ...
  const content = (
    ...
    >
      {headline !== undefined && (
        <div ref={headlineRef} className="will-change-transform [backface-visibility:hidden]">
          {headline}
        </div>
      )}
      {children !== undefined && (
  ...
  return (
    <section ref={rootRef} className={cn('relative w-full', className)} style={rootStyle}>
      <div
        ref={stageRef}
            ...
            {images.map((src, i) => (
              ...
              >
                <img
                  ...
                  draggable={false}
                  loading="lazy"
                  className={cn('h-full w-full select-none object-cover', grayscale && 'grayscale')}
                />
              </div>
            ))}
          </div>
