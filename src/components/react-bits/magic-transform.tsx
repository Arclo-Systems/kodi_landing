...
const ResultBody = () => (
  <div className="flex flex-col gap-[3px] rounded-md border border-neutral-200 bg-white p-2 shadow-[0_2px_8px_rgba(0,0,0,0.04)] dark:border-[var(--borde)] dark:bg-[var(--tarjeta)]">
        ...
        >
          {documents.map((doc, i) => (
            <SlidingDoc
              ...
              paused={paused}
              className={cn(
                // El gris neutro de fábrica no es el nuestro: en oscuro las
                // hojas van con la superficie de tarjeta del sitio, que es la
                // misma del resto de las secciones.
                'rounded-[14px] border border-neutral-200 bg-white shadow-[0_8px_32px_rgba(0,0,0,0.06)] dark:border-[var(--borde)] dark:bg-[var(--tarjeta)]',
                classNames?.document,
              )}
            />
          ))}
        ...
        >
          {results.map((res, i) => {
            ...
            return (
              <motion.div
                key={res.id}
                className={cn('absolute flex w-[170px] flex-col gap-1.5', classNames?.result)}
                style={{
                  left: 0,
                  top: 0,
